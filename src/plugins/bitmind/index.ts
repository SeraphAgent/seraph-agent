...
const detectImage: Action = {
    ...
    handler: async (
        ...
        if (message.content.skipBitMindAction) {
            return null;
        }
        ...
        try {
            ...
            const responseText = `According to BitMind's Deepfake Detection Subnet, the image ${
              result.isAI ? 'appears to be' : 'does not appear to be'} AI-generated 
              (${Math.round(result.confidence * 100)}% confidence of AI generation).`;
